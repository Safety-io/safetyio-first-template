@include('layouts.app')
@php use App\Http\Controllers\BlogController;use App\Models\Blog;use Illuminate\Support\Facades\DB; @endphp
        ...
        <main>
            ...
            <section class="pt-14 pb-17.5">
                <div class="max-w-[1170px] mx-auto px-4 sm:px-8 xl:px-0">
                    <div>
                        <div class="flex flex-wrap  text-left mb-12">
                            <div class="w-full lg:w-6/12 px-4">
                                ...
                                <!-- Description -->
                                <p class="text-gray-700 text-lg h-1 font-light pb-7">
                                    Nous vous permettons de rester informés de toutes les dernières nouveautés
                                    technologiques et
                    ...
                    <div class="pt-5 border-t border-gray-3">
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-11 gap-x-7.5">

                            {{--ALL BLOG ARTICLE'S FILTER--}}
                            <a href="{{ route('pages.blog') }}" class="group">
                                <div
                                    class="border border-gray-3 rounded-[20px] {{ !isset($selected_category) ? "bg-red-50": "bg-gray-50"}} p-5 group-hover:bg-white group-hover:drop-shadow-1 group-hover:-translate-y-2 transition-all">
                                    <div class="flex flex-wrap items-center gap-8">
                                        <div class="w-12 h-12 rounded-full overflow-hidden">
                                            <img src="{{ asset('/images/BlogTri/aut.png') }}"
                                                 alt="user"/>
                                        </div>
                                        <div>
                            ...
                            {{--RECORDERED CATEGORIES FILTER--}}
                            @foreach( $categories as $category)
                                @php
                                    $categories_color_schem = BlogController::categoryHilightingColor($category->name);
                                @endphp
                                <a href="{{ route('blog.category', ['name' => $category->name]) }}" class="group">
                                    <div
                                        class="border border-gray-3 rounded-[20px] {{ isset($selected_category) && $selected_category == $category->name ? $categories_color_schem['bg']: "bg-gray-50"}} p-5 group-hover:bg-white group-hover:drop-shadow-1 group-hover:-translate-y-2 transition-all">
                                        <div class="flex flex-wrap items-center gap-8">
                                            <div class="w-12 h-12 rounded-full overflow-hidden">
                                                <img src="{{ asset('/images/BlogTri/'.$category->banner) }}"
                                                     alt="user"/>
                                            </div>
                                            <div>
                                                <h3 class="font-semibold text-custom-xl text-dark mb-1">
                    ...
                    <div
                        ...
                        {{--CUSTUM TEXT AND BACKGROUND COLOR BY CATEGORY--}}
                        @foreach($blogs as $blog)
                            @php
                                $category = Blog::find($blog->category_id)->category->name;
                                $categories_color_schem = BlogController::categoryHilightingColor($category);
                                $background_color = $categories_color_schem['bg'];
                                $text_color = $categories_color_schem['txt'];
/*
                                switch ($category) {
                                    case "Reseaux": {
                            ...
                            <div class="group">
                                ...
                                <h4>
                                    <a href="{{ route('readBlog', ['id' => $blog->id]) }}"
                            ...
                            {{ $blog->title }}
                                ...
                                <div class="flex flex-wrap gap-3 items-center justify-between mt-4.5">
                                    ...
                                    <span
                                        class="inline-flex {{ $text_color }} {{ $background_color }} font-medium text-sm py-1 px-3 rounded-full">{{ DB::table('categories')->where('id', $blog->category_id)->first()->name }}</span>
                                </div>
                            </div>
                        @endforeach
